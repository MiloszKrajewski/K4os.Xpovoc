...
{
	private readonly ConcurrentQueue<SqliteConnection> _pool = new();
	...
	{
		SqlJob? ToJob(JobRec rec) =>
			rec.payload is null ? null : new SqlJob(
				rec.row_id, Guid.Parse(rec.job_id.Required()),

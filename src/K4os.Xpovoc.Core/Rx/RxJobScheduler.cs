...
public class RxJobScheduler: IJobScheduler
{
	...
	private readonly LinkedList<RxJob> _jobs = new();
	...
	private async Task Handle(LinkedListNode<RxJob> node)
	{
		...
		try
		{
			...
			{
				Log.LogWarning("Job {Job} has no payload and has been ignored", guid);
			}
		}
		catch (Exception e)
		{
			...
			CreateAndScheduleEntry(guid, payload.Required(), when, attempt);
		}
	}
